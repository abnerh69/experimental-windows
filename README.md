...
- **`showDialog` como ventana crea un sheet invisible que bloquea la app** (sin botón rojo, `Quit` pita): la variante *sized-to-content* del diálogo no se presenta en macOS. El demo incluye un watchdog que lo autocancela a los 6 s, más las variantes `fullscreen` (tamaño fijo, funciona) y clásica (`DialogRoute` manual); análisis en [`docs/showdialog-macos.md`](docs/showdialog-macos.md).
